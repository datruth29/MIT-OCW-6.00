...
def subStringMatchExactlyOneSub(target, key):
  ...
  oneSubMatches = subStringMatchOneSub(key, target)
  for a in oneSubMatches:
    if a not in exactMatches:
      oneSubExactMatches += (a, )
  return oneSubExactMatches
...
for t in range(0, len(targets)):
  for k in range(0, len(keys)):
    ...
    print subStringMatchExact(targets[t], keys[k])
    print subStringMatchOneSub(keys[k], targets[t])
    print subStringMatchExactlyOneSub(targets[t],keys[k])
